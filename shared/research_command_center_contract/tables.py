"""Minimum shared table contracts.

These definitions are intentionally compact. Agent checkpoints can propose
amendments before implementation when a shared field is needed.
"""

from dataclasses import dataclass


@dataclass(frozen=True)
class TableContract:
    name: str
    storage_layer: str
    columns: tuple[str, ...]
    primary_entity_id: str | None = None
    is_time_series: bool = False


APP_METADATA_TABLES: tuple[TableContract, ...] = (
    TableContract(
        name="research_programs",
        storage_layer="postgres",
        columns=(
            "program_id",
            "program_name",
            "program_description",
            "problem_statement",
            "initiating_context",
            "research_goal",
            "hypothesis",
            "research_objectives",
            "status",
            "research_area",
            "current_focus",
            "owner_name",
            "researcher_names_json",
            "tags_json",
            "linked_datasets_json",
            "linked_experiment_ids_json",
            "linked_run_ids_json",
            "notes_json",
            "input_source",
            "created_at",
            "updated_at",
            "created_by_user_id",
            "updated_by_user_id",
        ),
        primary_entity_id="program_id",
    ),
    TableContract(
        name="experiments",
        storage_layer="postgres",
        columns=(
            "experiment_id",
            "program_id",
            "experiment_name",
            "experiment_description",
            "research_question",
            "hypothesis",
            "experiment_type",
            "status",
            "owner_name",
            "evaluation_plan",
            "tags_json",
            "variants_json",
            "notes_json",
            "linked_datasets_json",
            "linked_run_ids_json",
            "linked_model_version_ids_json",
            "decision_notes",
            "input_source",
            "created_at",
            "updated_at",
            "created_by_user_id",
            "updated_by_user_id",
        ),
        primary_entity_id="experiment_id",
    ),
    TableContract(
        name="dataset_versions",
        storage_layer="postgres",
        columns=(
            "dataset_id",
            "dataset_version_id",
            "name",
            "version",
            "status",
            "source_priority",
            "raw_uri",
            "parquet_uri",
            "schema_uri",
            "parent_dataset_version_id",
            "created_at",
            "created_by_user_id",
        ),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="datasets",
        storage_layer="postgres",
        columns=(
            "dataset_id",
            "name",
            "description",
            "category",
            "default_task_type",
            "source_label",
            "owner_user_id",
            "created_at",
            "updated_at",
        ),
        primary_entity_id="dataset_id",
    ),
    TableContract(
        name="dataset_ingestion_jobs",
        storage_layer="postgres",
        columns=(
            "dataset_ingestion_job_id",
            "dataset_id",
            "source_dataset_name",
            "source_config_json",
            "status",
            "started_at",
            "ended_at",
            "records_seen",
            "records_written",
            "error_message",
            "created_by_user_id",
        ),
        primary_entity_id="dataset_ingestion_job_id",
    ),
    TableContract(
        name="run_configs",
        storage_layer="postgres",
        columns=("run_config_id", "dataset_version_id", "model_id", "config_uri", "created_at"),
        primary_entity_id="run_config_id",
    ),
    TableContract(
        name="training_runs",
        storage_layer="postgres",
        columns=(
            "run_id",
            "program_id",
            "experiment_id",
            "run_config_id",
            "dataset_version_id",
            "status",
            "started_at",
            "ended_at",
            "created_by_user_id",
        ),
        primary_entity_id="run_id",
    ),
    TableContract(
        name="checkpoints",
        storage_layer="postgres",
        columns=(
            "checkpoint_id",
            "run_id",
            "dataset_version_id",
            "step",
            "status",
            "artifact_uri",
            "created_at",
        ),
        primary_entity_id="checkpoint_id",
    ),
    TableContract(
        name="model_versions",
        storage_layer="postgres",
        columns=(
            "model_id",
            "model_version_id",
            "checkpoint_id",
            "run_id",
            "program_id",
            "experiment_id",
            "dataset_id",
            "dataset_version_id",
            "status",
            "artifact_uri",
            "created_at",
            "created_by_user_id",
        ),
        primary_entity_id="model_version_id",
    ),
    TableContract(
        name="eval_suites",
        storage_layer="postgres",
        columns=(
            "eval_suite_id",
            "program_id",
            "experiment_id",
            "name",
            "version",
            "status",
            "case_source_uri",
            "source_priority",
            "created_at",
        ),
        primary_entity_id="eval_suite_id",
    ),
    TableContract(
        name="eval_cases",
        storage_layer="postgres",
        columns=(
            "eval_case_id",
            "eval_suite_id",
            "case_name",
            "prompt_text",
            "expected_topics_json",
            "required_sections_json",
            "rubric_json",
            "tags_json",
            "created_at",
        ),
        primary_entity_id="eval_case_id",
    ),
    TableContract(
        name="eval_runs",
        storage_layer="postgres",
        columns=(
            "eval_run_id",
            "eval_suite_id",
            "program_id",
            "experiment_id",
            "model_version_id",
            "run_id",
            "checkpoint_id",
            "dataset_id",
            "dataset_version_id",
            "status",
            "source_priority",
            "started_at",
            "ended_at",
            "created_by_user_id",
            "evaluator_name",
            "evaluator_version",
            "eval_job_uri",
            "external_eval_run_id",
            "git_commit",
            "environment_json",
            "notes",
        ),
        primary_entity_id="eval_run_id",
    ),
    TableContract(
        name="eval_failures",
        storage_layer="postgres",
        columns=(
            "eval_failure_id",
            "eval_run_id",
            "eval_output_id",
            "eval_case_id",
            "model_version_id",
            "dataset_version_id",
            "severity",
            "failure_type",
            "failure_reason",
            "status",
            "created_at",
        ),
        primary_entity_id="eval_failure_id",
    ),
    TableContract(
        name="dataset_candidates",
        storage_layer="postgres",
        columns=(
            "dataset_candidate_id",
            "eval_failure_id",
            "source_eval_run_id",
            "source_eval_output_id",
            "source_model_version_id",
            "program_id",
            "experiment_id",
            "target_dataset_id",
            "failure_type",
            "status",
            "proposed_input_text",
            "proposed_target_text",
            "review_notes",
            "source_priority",
            "created_at",
            "created_by_user_id",
            "reviewed_at",
            "reviewed_by_user_id",
            "included_dataset_id",
            "included_dataset_version_id",
            "included_at",
        ),
        primary_entity_id="dataset_candidate_id",
    ),
    TableContract(
        name="dataset_iterations",
        storage_layer="object_store_manifest",
        columns=(
            "dataset_id",
            "dataset_version_id",
            "parent_dataset_version_id",
            "candidate_status",
            "candidate_count",
            "included_candidate_ids_json",
            "source_eval_failure_ids_json",
            "source_model_version_ids_json",
            "created_at",
            "created_by_user_id",
        ),
        primary_entity_id="dataset_version_id",
    ),
)

ANALYTICAL_TABLES: tuple[TableContract, ...] = (
    TableContract(
        name="training_metrics",
        storage_layer="parquet_duckdb",
        columns=("run_id", "timestamp", "step", "metric_name", "metric_value"),
        primary_entity_id="run_id",
        is_time_series=True,
    ),
    TableContract(
        name="compute_metrics",
        storage_layer="parquet_duckdb",
        columns=("run_id", "node_id", "gpu_id", "timestamp", "step", "metric_name", "metric_value"),
        primary_entity_id="run_id",
        is_time_series=True,
    ),
    TableContract(
        name="eval_outputs",
        storage_layer="parquet_duckdb",
        columns=(
            "eval_output_id",
            "eval_run_id",
            "eval_case_id",
            "model_version_id",
            "dataset_id",
            "dataset_version_id",
            "prompt_text",
            "output_text",
            "score",
            "scoring_method",
            "scores_json",
            "created_at",
        ),
        primary_entity_id="eval_run_id",
    ),
    TableContract(
        name="eval_scores",
        storage_layer="parquet_duckdb",
        columns=(
            "eval_run_id",
            "eval_output_id",
            "eval_case_id",
            "model_version_id",
            "metric_name",
            "metric_value",
            "scoring_method",
            "created_at",
        ),
        primary_entity_id="eval_run_id",
    ),
    TableContract(
        name="dataset_quality_reports",
        storage_layer="parquet_duckdb",
        columns=("dataset_version_id", "timestamp", "metric_name", "metric_value", "source_priority"),
        primary_entity_id="dataset_version_id",
        is_time_series=True,
    ),
    TableContract(
        name="dataset_records",
        storage_layer="parquet_duckdb",
        columns=(
            "record_id",
            "dataset_id",
            "dataset_version_id",
            "source_dataset_name",
            "source_split",
            "source_row_id",
            "category",
            "task_type",
            "input_text",
            "instruction",
            "context",
            "question",
            "chosen_text",
            "rejected_text",
            "target_text",
            "response_text",
            "prompt_messages_json",
            "metadata_json",
            "content_hash",
            "source_label",
            "created_at",
        ),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="dataset_schema_profiles",
        storage_layer="parquet_duckdb",
        columns=(
            "dataset_version_id",
            "field_name",
            "field_type",
            "non_null_count",
            "null_count",
            "empty_count",
            "distinct_count",
            "min_length",
            "max_length",
            "mean_length",
            "example_values_json",
            "created_at",
        ),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="dataset_duplicate_reports",
        storage_layer="parquet_duckdb",
        columns=("dataset_version_id", "content_hash", "duplicate_count", "record_ids_json", "created_at"),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="dataset_pii_scan_results",
        storage_layer="parquet_duckdb",
        columns=(
            "dataset_version_id",
            "record_id",
            "pii_type",
            "match_count",
            "scanner_name",
            "scanner_version",
            "created_at",
        ),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="dataset_token_statistics",
        storage_layer="parquet_duckdb",
        columns=(
            "dataset_version_id",
            "record_id",
            "tokenizer_name",
            "input_token_count",
            "target_token_count",
            "total_token_count",
            "created_at",
        ),
        primary_entity_id="dataset_version_id",
    ),
    TableContract(
        name="dataset_lineage",
        storage_layer="parquet_duckdb",
        columns=(
            "dataset_id",
            "source_dataset_version_id",
            "target_dataset_version_id",
            "lineage_event_type",
            "transform_name",
            "transform_config_uri",
            "created_at",
            "created_by_user_id",
        ),
        primary_entity_id="dataset_id",
    ),
    TableContract(
        name="dataset_usage",
        storage_layer="parquet_duckdb",
        columns=(
            "dataset_version_id",
            "run_id",
            "model_version_id",
            "eval_run_id",
            "usage_type",
            "created_at",
        ),
        primary_entity_id="dataset_version_id",
    ),
)


def table_by_name(name: str) -> TableContract:
    for table in (*APP_METADATA_TABLES, *ANALYTICAL_TABLES):
        if table.name == name:
            return table
    raise KeyError(f"Unknown shared table contract: {name}")
